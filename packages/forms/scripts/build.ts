...
console.log("Building @opentui/forms library...")
...
const buildResult = await Bun.build({
  entrypoints: [join(rootDir, packageJson.module), join(rootDir, "src/react.ts"), join(rootDir, "src/solid.ts")],
  target: "bun",
  format: "esm",
  outdir: distDir,
  sourcemap: "linked",
...
if (existsSync(corePackageJsonPath)) {
  ...
  if (coreBuildResult.status !== 0) {
    console.error("Error: Failed to build @opentui/core declarations required by @opentui/forms")
    process.exit(1)
  }
}

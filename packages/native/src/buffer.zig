        ...
        for (virtual_lines[firstVisibleLine..lastPossibleLine], 0..) |vline, slice_idx| {
            ...
            for (vline.chunks.items) |vchunk| {
                ...
                text_buffer_loop: while (byte_offset < byte_end and col < col_end) {
                    ...
                    // Resolve metadata once per printable ASCII style/chunk run.
                        ...
                        !vline.is_truncated and view.getSelection() == null and next_change_col > source_col_pos)
                    {
                        ...
                        if (run.len > 1) {
                            ...
                            for (run, 0..) |char, offset| {
                                if (useTransparentTextFastPath and self.trySetTransparentTextCellFast(index + @as(u32, @intCast(offset)), char, drawFg, drawAttributes)) continue;
                                self.setCellWithAlphaBlendingCell(@intCast(currentX + @as(i32, @intCast(offset))), @intCast(currentY), makeCell(char, drawFg, drawBg, drawAttributes));
                            }

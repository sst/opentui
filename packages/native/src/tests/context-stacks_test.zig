...
test "Context checked stacks ABI rejects invalid calls without writing output or changing state" {
    ...
    var output: f32 = -1;
    const opacity: f32 = 0;
    const nan_opacity = std.math.nan(f32);
    const half: f32 = 0.5;
    try testing.expectEqual(c.OT_INVALID_ARGUMENT, abi.ot_buffer_stack(null, &id, null, 0, 0, 0, 0, 0, &opacity, &output));
    try testing.expectEqual(c.OT_INVALID_ARGUMENT, abi.ot_buffer_stack(owner, null, null, 0, 0, 0, 0, 0, &opacity, &output));
    try testing.expectEqual(c.OT_INVALID_ARGUMENT, abi.ot_buffer_stack(owner, &id, null, 0, 0, 0, 0, 0, &opacity, null));
    try testing.expectEqual(c.OT_INVALID_ARGUMENT, abi.ot_buffer_stack(owner, &id, null, 0, 0, 0, 0, 0, null, &output));
    try testing.expectEqual(c.OT_INVALID_ARGUMENT, abi.ot_buffer_stack(owner, &id, null, 7, 0, 0, 0, 0, &opacity, &output));
    try testing.expectEqual(c.OT_INVALID_ARGUMENT, abi.ot_buffer_stack(owner, &id, null, c.OT_BUFFER_STACK_PUSH_OPACITY, 0, 0, 0, 0, &nan_opacity, &output));
    try testing.expectEqual(c.OT_INVALID_ARGUMENT, abi.ot_buffer_stack(owner, &id, null, c.OT_BUFFER_STACK_PUSH_SCISSOR, std.math.maxInt(i32), 0, 1, 1, &opacity, &output));
    try testing.expectEqual(@as(f32, -1), output);
    try testing.expectEqual(c.OT_OK, abi.ot_buffer_stack(owner, &id, null, c.OT_BUFFER_STACK_PUSH_OPACITY, 0, 0, 0, 0, &half, &output));
    ...
    try testing.expectEqual(c.OT_STALE_HANDLE, abi.ot_buffer_stack(owner, &id, null, 0, 0, 0, 0, 0, &opacity, &output));

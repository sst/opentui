...
export const DocumentRangeInputStruct = defineStruct(
  ...
  ],
  {
    mapValue: (
      ...
      ...range,
      startChunk: range.remove ? 0 : range.startChunk,
      endChunk: range.remove ? 0 : range.endChunk,
      attributes: range.remove ? 0 : range.attributes,
      styleId: range.remove ? 0 : range.styleId,
      styleKind: !range.remove && typeof range.styleId === "number" ? 1 : 0,
      syntaxStyle: range.remove ? 0 : range.syntaxStyle,
      remove: range.remove ? 1 : 0,
      styled: range.remove || range.styled === false ? 0 : 1,
      priority: range.remove ? 0 : range.priority,
      fg: range.remove ? null : (normalizeColorValue(range.fg ?? null)?.rgba ?? null),
      bg: range.remove ? null : (normalizeColorValue(range.bg ?? null)?.rgba ?? null),
      link: range.remove ? "" : typeof range.link === "object" && range.link ? range.link.url : range.link,
    }),
  },
)

...
pub const CliRenderer = struct {
    ...
    fn applyPendingSplitFooterTransition(self: *CliRenderer, writer: anytype, frame_started: *bool) void {
        ...
        if (!frame_started.*) {
            beginRenderFrameWithLinkReset(writer, self.terminal.getCapabilities().hyperlinks);
            frame_started.* = true;
        }
        ...
        if (begin_frame) {
            ...
            switch (self.backend) {
                inline else => |*b| {
                    b.beginFrame();
                    var w = b.writer();
                    beginRenderFrameWithLinkReset(&w, self.terminal.getCapabilities().hyperlinks);
    ...
    /// Like beginRenderFrame but first emits a defensive OSC 8 close so any
    /// hyperlink left open by a prior torn/dropped write cannot bleed into
    /// this frame's cells. Only emitted when the terminal supports hyperlinks.
    fn beginRenderFrameWithLinkReset(writer: anytype, hyperlinks_enabled: bool) void {

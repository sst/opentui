...
test "EditorView - mouse selection scrolls viewport down when dragging below" {
    const pool = gp.initGlobalPool(std.testing.allocator);
    defer gp.deinitGlobalPool();
    ...
    _ = ev.setLocalSelection(0, 0, 5, 45, null, null);
    ...
    try std.testing.expect(vp_after.y > 30);

    // Cursor should be at the focus position (line 45, col 5)
    try std.testing.expectEqual(@as(u32, 45), cursor_after.row);
    try std.testing.expect(cursor_after.col <= 7); // Line text is "Line 45", so col should be clamped

    // Cursor should be visible in viewport
    try std.testing.expect(cursor_after.row >= vp_after.y);
    try std.testing.expect(cursor_after.row < vp_after.y + vp_after.height);
}

test "EditorView - mouse selection scrolls viewport up when dragging above" {
    const pool = gp.initGlobalPool(std.testing.allocator);
    defer gp.deinitGlobalPool();
    ...
    // Cursor is at end (line 49), so viewport should be scrolled down
    _ = ev.getVirtualLines();

    const vp_initial = ev.getViewport().?;
    try std.testing.expect(vp_initial.y > 35);

    // Simulate mouse selection from current viewport position dragging up off-screen
    // Since focus is above the viewport, focusY will be negative
    // The selection will be clamped to line 0 (start of buffer)
    const anchor_y: i32 = @intCast(49 - vp_initial.y);
    const focus_y: i32 = -10; // Way above viewport

    _ = ev.setLocalSelection(0, anchor_y, 5, focus_y, null, null);
    ...
    // Viewport should have scrolled up significantly
    try std.testing.expect(vp_after.y < vp_initial.y);

    // Cursor should be at line 0 (where the selection was clamped to)
...
test "EditorView - updateLocalSelection continues scrolling as mouse moves" {
    const pool = gp.initGlobalPool(std.testing.allocator);
    defer gp.deinitGlobalPool();
    ...
    _ = ev.setLocalSelection(0, 0, 5, 0, null, null);
    _ = ev.getVirtualLines();
    ...
    _ = ev.updateLocalSelection(0, 0, 5, 15, null, null);
    _ = ev.getVirtualLines();

    const vp_mid = ev.getViewport().?;
    try std.testing.expect(vp_mid.y > 0);

    const cursor_mid = ev.getPrimaryCursor();
    try std.testing.expectEqual(@as(u32, 15), cursor_mid.row);

    // Extend selection further to line 45
    _ = ev.updateLocalSelection(0, 0, 5, 45, null, null);
    _ = ev.getVirtualLines();

    const vp_end = ev.getViewport().?;
    const cursor_end = ev.getPrimaryCursor();

    // Viewport should have scrolled even further
    try std.testing.expect(vp_end.y > vp_mid.y);

    // Cursor should be at line 49 (end of buffer - selection gets clamped)
    try std.testing.expectEqual(@as(u32, 49), cursor_end.row);

    // Cursor should be visible
    try std.testing.expect(cursor_end.row >= vp_end.y);
    try std.testing.expect(cursor_end.row < vp_end.y + vp_end.height);
}

test "EditorView - mouse selection with wrapping scrolls correctly" {
    const pool = gp.initGlobalPool(std.testing.allocator);
    defer gp.deinitGlobalPool();
    ...
    // Create text that will wrap into many virtual lines (at 20 chars/line, this creates ~100+ vlines)
    const long_text = "" ++ "" ++ "" ++ "";
    try eb.setText(long_text);
    ...
    // Total virtual lines should be much more than 10
    const total_vlines = ev.getTotalVirtualLineCount();
    try std.testing.expect(total_vlines > 50);

    // Simulate mouse drag to virtual line 50 (if it exists)
    const target_vline = @min(50, total_vlines - 1);
    _ = ev.setLocalSelection(0, 0, 10, @intCast(target_vline), null, null);
...
test "EditorView - mouse selection doesn't scroll when focus is within viewport" {
    ...
    _ = ev.setLocalSelection(0, 0, 5, 5, null, null);
    _ = ev.getVirtualLines();
...
test "EditorView - mouse selection focus outside buffer bounds clamps correctly" {
    ...
    _ = ev.setLocalSelection(0, 0, 5, 100, null, null);
    _ = ev.getVirtualLines();

...
test "TextBufferView word wrapping - segment merging at wrap boundary basic" {
    const pool = gp.initGlobalPool(std.testing.allocator);
    defer gp.deinitGlobalPool();
    ...
    // Reproduce: typing "hello world ddddddddd" with wrap width 17
    // Expected: "hello world " on line 0, "ddddddddd" on line 1
    try tb.setText("hello world ddddddddd");
    ...
    std.debug.print("\n=== TEST: Basic word wrap at boundary ===\n", .{});
    ...
    for (vlines, 0..) |vline, i| {
        std.debug.print("VLine {}: width={} char_offset={} source_col_offset={}\n", .{ i, vline.width, vline.char_offset, vline.source_col_offset });
    }

    // Should have 2 virtual lines
    try std.testing.expectEqual(@as(usize, 2), vlines.len);

    // Line 0 should be "hello world " (12 chars)
...
test "TextBufferView word wrapping - segment merging with fragmented rope" {
    const pool = gp.initGlobalPool(std.testing.allocator);
    defer gp.deinitGlobalPool();
    ...
    // Simulate fragmented rope like what happens during typing
    // "hello world ddddddddd" but built char by char
    const text = "hello world ddddddddd";
    const mem_id = try tb.mem_registry.register(text, false);
    ...
    // Fragment the text to simulate typing
    // "hello world " as one chunk, then each 'd' as separate chunks
    const chunk1 = tb.createChunk(mem_id, 0, 12); // "hello world "
    try segments.append(Segment{ .text = chunk1 });

    // Add each 'd' as a separate chunk
    var i: u32 = 12;
    while (i < 21) : (i += 1) {
        const chunk = tb.createChunk(mem_id, i, i + 1);
    ...
    std.debug.print("\n=== TEST: Fragmented rope word wrap ===\n", .{});
    std.debug.print("Text: 'hello world ddddddddd' (fragmented)\n", .{});
    std.debug.print("Wrap width: 17\n", .{});
    std.debug.print("Virtual lines count: {}\n", .{vlines.len});
    for (vlines, 0..) |vline, idx| {
        std.debug.print("VLine {}: width={} char_offset={} source_col_offset={}\n", .{ idx, vline.width, vline.char_offset, vline.source_col_offset });
    }
...
test "TextBufferView word wrapping - typing simulation with backspace" {
    const pool = gp.initGlobalPool(std.testing.allocator);
    defer gp.deinitGlobalPool();
    ...
    // First, type some text with backspace before reaching the boundary
    try tb.setText("hello worldx");

    std.debug.print("\n=== TEST: After typing 'hello worldx' ===\n", .{});
    var vlines = view.getVirtualLines();
    std.debug.print("Virtual lines count: {}\n", .{vlines.len});
    for (vlines, 0..) |vline, idx| {
        std.debug.print("VLine {}: width={} char_offset={} source_col_offset={}\n", .{ idx, vline.width, vline.char_offset, vline.source_col_offset });
    ...
    // Now "backspace" by removing the 'x' and adding space and d's
    try tb.setText("hello world ddddddddd");

    std.debug.print("\n=== TEST: After changing to 'hello world ddddddddd' ===\n", .{});
    vlines = view.getVirtualLines();
    std.debug.print("Virtual lines count: {}\n", .{vlines.len});
    for (vlines, 0..) |vline, idx| {
        std.debug.print("VLine {}: width={} char_offset={} source_col_offset={}\n", .{ idx, vline.width, vline.char_offset, vline.source_col_offset });
...
test "TextBufferView word wrapping - continuous word across boundary from same mem_id" {
    const pool = gp.initGlobalPool(std.testing.allocator);
    defer gp.deinitGlobalPool();
    ...
    // Create chunks from same memory that should be merged
    const text = "hello world ddddddddd";
    const mem_id = try tb.mem_registry.register(text, false);
    ...
    // Create contiguous chunks from the same mem_id
    // "hello world "
    const chunk1 = tb.createChunk(mem_id, 0, 12);
    try segments.append(Segment{ .text = chunk1 });

    // "d" - contiguous with previous
    const chunk2 = tb.createChunk(mem_id, 12, 13);
    try segments.append(Segment{ .text = chunk2 });

    // "dddddddd" - contiguous with previous
    const chunk3 = tb.createChunk(mem_id, 13, 21);
    try segments.append(Segment{ .text = chunk3 });
    ...
    std.debug.print("\n=== TEST: Contiguous chunks from same mem_id ===\n", .{});
    std.debug.print("Chunks: chunk1[0:12]='hello world ', chunk2[12:13]='d', chunk3[13:21]='dddddddd'\n", .{});

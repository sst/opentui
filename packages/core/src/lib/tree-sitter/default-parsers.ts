...
// Last generated: 2026-05-05T10:40:40.053Z
...
import go_highlights from "./assets/go/highlights.scm" with { type: "file" }
import go_language from "./assets/go/tree-sitter-go.wasm" with { type: "file" }
  ...
  if (!_cachedParsers) {
    _cachedParsers = [
      ...
      },
      {
        ...
        aliases: ["golang"],
        queries: {
          highlights: [resolve(dirname(fileURLToPath(import.meta.url)), go_highlights)],
        },
        wasm: resolve(dirname(fileURLToPath(import.meta.url)), go_language),
      },
    ]
  }

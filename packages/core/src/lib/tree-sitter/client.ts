...
export class TreeSitterClient extends EventEmitter<TreeSitterClientEvents> {
  ...
  private _touchOneshotCache(key: string): void {
    const v = this._oneshotCache.get(key)
    if (v === undefined) return
    this._oneshotCache.delete(key)
    this._oneshotCache.set(key, v)
  }
  private _setOneshotCache(key: string, value: SimpleHighlight[]): void {
    if (this._oneshotCache.has(key)) {
      this._oneshotCache.delete(key)
    } else if (this._oneshotCache.size >= TreeSitterClient.ONESHOT_CACHE_LIMIT) {

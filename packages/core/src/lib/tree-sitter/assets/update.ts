...
async function generateDefaultParsersFile(parsers: GeneratedParser[], outputPath: string): Promise<void> {
  const assetPaths = parsers
    .map((parser) => {
      ...
      const lines = [
        ...
        `  ${JSON.stringify(toAssetKey(parser.highlightsPath))},`,
        ...
        `  ${JSON.stringify(toAssetKey(parser.languagePath))},`,
      ...
      if (parser.injectionsPath) {
        lines.push(
          ...
          `  ${JSON.stringify(toAssetKey(parser.injectionsPath))},`,
  ...
  const parserDefinitions = generateParserDefinitions(parsers)
...
`
  ...
  const nodeOutputPath = outputPath.replace(/\.ts$/, ".node.ts")
  const manifestOutputPath = path.join(path.dirname(outputPath), "default-parser-assets.ts")
  await mkdir(path.dirname(outputPath), { recursive: true })
  await Promise.all([
    writeFile(outputPath, fileContent, "utf-8"),
    writeFile(nodeOutputPath, generateNodeDefaultParsersFile(parsers, parserDefinitions), "utf-8"),
    writeFile(manifestOutputPath, generateDefaultParserAssetsFile(parsers), "utf-8"),
  ])
  console.log(`Generated ${path.basename(outputPath)} with ${parsers.length} parsers`)
}

function generateParserDefinitions(parsers: GeneratedParser[]): string {
  return parsers
    .map((parser) => {
      const safeFiletype = parser.filetype.replace(/[^a-zA-Z0-9]/g, "_")
...
function generateNodeDefaultParsersFile(parsers: GeneratedParser[], parserDefinitions: string): string {
  const assetPaths = parsers
    .map((parser) => {
      const safeFiletype = parser.filetype.replace(/[^a-zA-Z0-9]/g, "_")
      const lines = [
        generateNodeAssetPath(`${safeFiletype}_highlights`, parser.highlightsPath),
        generateNodeAssetPath(`${safeFiletype}_language`, parser.languagePath),
      ]
      if (parser.injectionsPath) {
        lines.push(generateNodeAssetPath(`${safeFiletype}_injections`, parser.injectionsPath))
      }
      return lines.join("\n")
    })
    .join("\n")

  return `// This file is generated by assets/update.ts - DO NOT EDIT MANUALLY
// Run 'bun assets/update.ts' to regenerate this file
...
function generateNodeAssetPath(variableName: string, assetPath: string): string {
  return `const ${variableName} = resolveAssetPath(
  ${JSON.stringify(toAssetKey(assetPath))},
  new URL(${JSON.stringify(assetPath)}, import.meta.url),
)`
}

function generateDefaultParserAssetsFile(parsers: GeneratedParser[]): string {
  const paths = parsers.flatMap((parser) =>
...
export const defaultParserAssetPaths = ${JSON.stringify(paths, null, 2)} as const
`
}

function toAssetKey(assetPath: string): string {
  return `@opentui/core/${toPackageRelativeAssetPath(assetPath)}`
}

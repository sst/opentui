...
describe("TextareaRenderable", () => {
  ...
  describe("Mouse Event Stress Test", () => {
    ...
    it("STRESS TEST: concurrent mouse events on multiple textareas", async () => {
      const { textarea: editor1 } = await createTextareaRenderable(currentRenderer, {
        initialValue: "Editor 1",
        width: 40,
        height: 5,
      })

      const { textarea: editor2 } = await createTextareaRenderable(currentRenderer, {

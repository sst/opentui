...
const declarationPaths = ["index.d.ts", "testing.d.ts", "yoga.d.ts", "lib/tree-sitter/parser.worker.d.ts"]
...
assert.equal(typeof yoga.Node.create, "function")
...
for (const specifier of [${JSON.stringify(packageJson.name)}, ${JSON.stringify(`${packageJson.name}/testing`)}, ${JSON.stringify(`${packageJson.name}/yoga`)}, ${JSON.stringify(`${packageJson.name}/tree-sitter/update-assets`)}]) {
  assert.throws(
    () => require(specifier),

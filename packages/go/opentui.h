...
void setWriteTarget(CliRenderer* renderer, uint32_t target);
...
size_t renderIntoWriteBuffer(CliRenderer* renderer, bool force);
size_t getWriteBufferLength(CliRenderer* renderer);
size_t copyWriteBuffer(CliRenderer* renderer, uint8_t* dest, size_t maxLen);
size_t setupTerminalToBuffer(CliRenderer* renderer, bool useAlternateScreen);
size_t teardownTerminalToBuffer(CliRenderer* renderer);

...
`@lexwdex-org/solid` publishes one npm package with runtime-specific entrypoints behind the package `exports` map. The root package and JSX runtime subpaths work in both Bun and Node. Bun-specific helpers stay Bun-only and throw a clear runtime error if imported from Node.

| Entrypoint                                        | Bun | Node | Notes                                           |
| ------------------------------------------------- | --- | ---- | ----------------------------------------------- |
| `@lexwdex-org/solid`                                  | yes | yes  | Main renderer and test helpers                  |
| `@lexwdex-org/solid/jsx-runtime`                      | yes | yes  | Real runtime module for compiled JSX            |
| `@lexwdex-org/solid/jsx-dev-runtime`                  | yes | yes  | Real dev runtime module for compiled JSX        |
| `@lexwdex-org/solid/preload`                          | yes | no   | Bun preload hook                                |
| `@lexwdex-org/solid/bun-plugin`                       | yes | no   | Bun bundler/plugin entry                        |
| `@lexwdex-org/solid/runtime-plugin-support`           | yes | no   | Bun runtime-loader support                      |
| `@lexwdex-org/solid/runtime-plugin-support/configure` | yes | no   | Bun runtime-loader support without side effects |
...
export default {
  ...
  presets: [[solid, { moduleName: "@lexwdex-org/solid", generate: "universal" }], [ts]],
}
```
...
```ts
import "@lexwdex-org/solid/runtime-plugin-support"
```
...
```tsx
import { writeSolidToScrollback } from "@lexwdex-org/solid"
...
```tsx
import { createScrollbackWriter } from "@lexwdex-org/solid"
...
```tsx
import { onResize } from "@lexwdex-org/solid"
...
```tsx
import { onFocus } from "@lexwdex-org/solid"
...
```tsx
import { onBlur } from "@lexwdex-org/solid"
...
```tsx
import { useTerminalDimensions } from "@lexwdex-org/solid"
...
```tsx
import { usePaste } from "@lexwdex-org/solid"
...
```tsx
import { useSelectionHandler } from "@lexwdex-org/solid"
...
```tsx
import { useTimeline } from "@lexwdex-org/solid"
...
```tsx
import { Portal, useRenderer } from "@lexwdex-org/solid"
...
If that executable loads external plugins/modules at runtime, keep `import "@lexwdex-org/solid/runtime-plugin-support"` in your app entry.

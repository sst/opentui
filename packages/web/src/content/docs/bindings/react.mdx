...
LaTeX math support is available from `@opentui/latex/react` and must be registered explicitly with `registerLatex()`.
...
LaTeX markdown transforms are registered separately with `registerLatexMarkdown()` from `@opentui/latex/markdown`.

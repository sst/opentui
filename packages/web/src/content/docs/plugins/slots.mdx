...
If you load plugin modules from disk at runtime, see the runtime support notes on the [React slots page](/docs/plugins/react#runtime-loaded-external-plugins) and [Solid slots page](/docs/plugins/solid#runtime-loaded-external-plugins). For custom hosts, use `@lexwdex-org/core/runtime-plugin-support` or `createRuntimePlugin` from `@lexwdex-org/core/runtime-plugin`.
...
```typescript
import type { PluginContext } from "@lexwdex-org/core"
...
```typescript
import { createSlotRegistry } from "@lexwdex-org/core"

...
This installs Bun runtime support so external TS/TSX plugin modules resolve against the host runtime instances (`@lexwdex-org/react`, React JSX runtime modules, and core runtime modules).
...
```ts
import { ensureRuntimePluginSupport } from "@lexwdex-org/react/runtime-plugin-support/configure"
...
First-party packages expose side-effect-free runtime module maps. For plugins that import `@lexwdex-org/three`, install `@lexwdex-org/three` in the host app and pass its runtime modules explicitly:
...
- `@lexwdex-org/react/runtime-plugin-support` — one-line runtime support for external plugin/module loading
- `@lexwdex-org/react/runtime-plugin-support/configure` — configurable runtime support without import-time side effects
...
import { createReactSlotRegistry, createRoot, Slot } from "@lexwdex-org/react"

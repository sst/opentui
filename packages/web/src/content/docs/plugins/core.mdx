...
- `@lexwdex-org/core/runtime-plugin-support` / `createRuntimePlugin` — runtime module support for external plugin/module loading in Bun
...
```ts
import "@lexwdex-org/core/runtime-plugin-support"
```

This installs Bun runtime support for `@lexwdex-org/core` plus default core runtime entrypoints (`@lexwdex-org/core/testing`). Add first-party package runtime modules explicitly when a plugin imports them.

If you need additional host-resolved modules, configure runtime support before the side-effect import is loaded:

```ts
const { ensureRuntimePluginSupport } = await import("@lexwdex-org/core/runtime-plugin-support/configure")
...
For plugins that import `@lexwdex-org/three`, install `@lexwdex-org/three` in the host app and pass its runtime module map:

```ts
import { runtimeModules as threeRuntimeModules } from "@lexwdex-org/three/runtime-modules"
import { ensureRuntimePluginSupport } from "@lexwdex-org/core/runtime-plugin-support/configure"
...
```typescript
import { createCliRenderer, createCoreSlotRegistry, resolveCoreSlot } from "@lexwdex-org/core"

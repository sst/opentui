...
Each file has a unique name, mode `0600`, and immutable contents. Files live in `TMPDIR`, or `/tmp` when it is unset. At most eight files and 64 MiB are outstanding per renderer. A busy image ID or exhausted budget uses raw without replacing a pending file. A matching Kitty reply releases a file. Frame events, stdout drain, and cursor reports do not acknowledge image consumption or presentation.

...
The renderable owns `image` and the `NativeImage` passed to `onLoad`; do not dispose or transfer them yourself. Source replacement, clearing, and recursive destruction release the image. `onError` reports load failures through `loadError`, and handled load failures resolve `loadPromise`. Exceptions thrown by `onLoad` or `onError` reject `loadPromise` after state has settled.

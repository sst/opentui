...
Set `kittyImageTransport` when you create the renderer. This changes Kitty image transmission, not protocol selection or image placement. All output still goes through the renderer's output backend.
...
- `"raw"` is the default. Opaque pixels use RGB24; other pixels use RGBA32.
- `"zlib"` tries native level-1 zlib compression. If compression expands the stream, exceeds the 64 MiB preparation limit, or cannot allocate its buffers, transmission falls back to raw. Existing encoded PNG data stays PNG; it is not recompressed.
- `"file"` opts into local POSIX file transport. Windows, remote sessions, disabled graphics, and multiplexers use raw instead. A custom stdout defaults to remote; set `remote: false` only when it really connects to a local terminal.

File mode tests both a real `t=f` query and an explicit-ID upload acknowledgement before sending application images by file. Old WezTerm versions that answer queries but omit upload acknowledgements remain on raw. The query is asynchronous, so initial frames can use raw while `fileState` is `"probing"`.

Each file has a unique name, mode `0600`, and immutable contents. Files use the `TMPDIR` forwarded at renderer creation, or `/tmp` if it is absent. Include `TMPDIR` if you override `forwardEnvKeys`. At most eight files and 64 MiB are outstanding per renderer. A busy image ID or exhausted budget uses raw without replacing a pending file. A matching Kitty reply releases a file. Frame events, stdout drain, and cursor reports do not acknowledge image consumption or presentation.

Leases have a five-second deadline, checked during rendering and once per second while idle. Expiry cancels the transport; it does not count as successful consumption. Timeout, terminal errors, output errors, `cancelKittyImageTransport()`, suspend, and destroy remove pending files and disable further file uploads. Active images retry through the raw/PNG path after a failed file upload. Late replies cannot re-enable file mode. Process termination that bypasses cleanup, such as `SIGKILL`, can leave files behind.

`kittyImageTransportStatus` reports `requested`, the last transmission's `effective` encoding (`raw`, `zlib`, `png`, or `file`), `fileState`, `fallback`, `pendingFiles`, and `pendingBytes`. The `compression` fallback covers compression that was unprofitable or unavailable. File mode preserves PNG payloads, but its effective transport is `file`. Scrollback snapshots always use self-contained raw/PNG output, or zlib when requested, because their sources are not retained for retries. Shared memory is not implemented.

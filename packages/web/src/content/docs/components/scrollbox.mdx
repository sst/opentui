...
Enable scroll anchoring when content can be inserted, removed, reordered, or resized above a manually scrolled viewport:
...
ScrollBox keeps the leading visible direct child at the same viewport position on each scrolled axis. Sticky scrolling and newer calls to `scrollTo`, `scrollBy`, or the scroll position setters take precedence over anchoring. Absolute and hidden children are not used as anchors.
...
| `preserveScrollAnchor`       | `boolean`                                | `false` | Preserve visible content during layout |

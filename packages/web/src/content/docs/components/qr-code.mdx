...
```typescript
import { QRCodeRenderable, createCliRenderer } from "@opentui/core"
...
Set `errorCorrectionLevel` with the core QR error correction enum:

```typescript
import { ErrorCorrectionLevel, QRCodeRenderable } from "@opentui/core"

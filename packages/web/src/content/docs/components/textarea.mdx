...
| `selectionOccupancy`     | `"cell"` or `"boundary"`              | `"cell"`           | Endpoint occupancy. Independent of `cursorStyle`. |
...
```typescript
textarea.setSelection(start, end) // already-exclusive [start, end)
textarea.setSelectionInclusive(start, end) // cell occupancy: extend end by grapheme width
textarea.selectAll()
...
Default occupancy is `cell`: both endpoint cells are selected, so the first shift+right selects two cells. Pair a bar
caret with `selectionOccupancy: "boundary"` explicitly. Cursor style never changes copied or deleted text.

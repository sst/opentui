...
function createLabeledInput(renderer: RenderContext, props: { id: string; label: string; placeholder: string }) {
  const row = new BoxRenderable(renderer, { flexDirection: "row", marginBottom: 1 })
  const input = new InputRenderable(renderer, {
    id: `${props.id}-input`,
  ...
  row.add(new TextRenderable(renderer, { content: props.label.padEnd(12), fg: "#888888" }))
  row.add(input)
  return { row, input }
}
...
const password = createLabeledInput(renderer, {
  id: "password",
  label: "Password:",

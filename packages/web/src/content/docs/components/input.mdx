...
Emitted on every keystroke as the value changes:
...
Emitted when the input loses focus (blur) or when Enter is pressed, but only if the value changed since focus:

---
title: Markdown
description: Render markdown with syntax-aware styling and optional math
order: 11
---

# Markdown

Render markdown content with syntax-aware styling, optional Tree-sitter highlighting for code blocks, tables, and opt-in LaTeX math.
...
Math rendering is opt-in because dollar signs are common in ordinary text and prices. Leave `math` unset or `false` to keep `$...$` content unchanged.
...
Pass `MarkdownMathOptions` when you need finer control:
...
Inline math uses `$...$` and renders inside normal paragraphs, headings, list items, and table cells. The parser skips escaped dollars, code spans, fenced code blocks, incomplete spans, and a closing `$` followed by a digit to reduce currency false positives.

Block math uses `$$...$$` or multiline blocks:
...
Display math is rendered as a [`LatexRenderable`](/docs/components/latex), participates in normal markdown block spacing, and can be overridden by `renderNode` when `token.type === "latex_block"`.

When `conceal` is `false`, inline math preserves `$` delimiters around the rendered expression. For example, `$x^2$` renders as `$x²$`. When `math` is disabled, the original LaTeX source is left untouched.
...
| `math`              | `boolean \| MarkdownMathOptions`                                                | `false`       | Enable inline and/or display LaTeX math rendering             |
...
| Option         | Type                                                      | Default | Description                                     |
| -------------- | --------------------------------------------------------- | ------- | ----------------------------------------------- |
| `inline`       | `boolean`                                                 | `true`  | Enable `$...$` inline math                      |
| `block`        | `boolean`                                                 | `true`  | Enable `$$...$$` display math blocks            |
| `latexOptions` | `Partial<Omit<LatexOptions, "content" \| "displayMode">>` | -       | Options passed to the underlying LaTeX renderer |

...
LaTeX math support is available from the separate `@opentui/latex` package. Register it before creating markdown renderables that should transform math delimiters:
...
The transform supports inline `$...$`, inline `\(...\)`, display `$$...$$`, and display `\[...\]`. Inline code and fenced code blocks are left unchanged.

See [LaTeX](/docs/components/latex) for the standalone renderable, JSX registration, supported notation, and sizing behavior.

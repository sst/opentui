...
A syntax-highlighted code display component powered by Tree-sitter. Supports many programming languages with accurate, fast highlighting.
...
The Code component uses Tree-sitter for parsing. Supported languages include:
...
Enable streaming mode for use cases like displaying LLM output where content arrives incrementally:
...
In streaming mode, the component optimizes highlighting for incremental updates.
...
| `wrapMode`     | `string`           | `"none"` | Text wrapping: `"none"`, `"char"`, `"word"` |

...
Use `SyntaxStyle.fromStyles(definitions, renderer.nativeScene)` to bind colors and attributes for syntax tokens.
Native style allocation requires an explicit owner. Standalone callers can use a
[`ResourceContext`](/docs/reference/buffer-api#create-and-destroy-a-buffer).

Registration snapshots the definitions, including their colors. `getStyle()`,
`getAllStyles()`, and `mergeStyles()` return detached values. To change a registered
style, call `registerStyle()` again. Changes to a previous input or getter result
do not update the registered style.
...
Share definitions between renderers, not resource lifetimes. Bind a copy to its
destination scene, and destroy it when that destination no longer needs it:
...
// After those consumers have been destroyed:
boundStyle.destroy()
```

`CodeRenderable` borrows the theme you supply. It does not destroy the theme.
Text and edit buffers require styles bound to their own owner.
A foreign attachment throws instead of creating a mirror.

Definition snapshots contain declared rules, not native-generated chunk styles.
Passing the definition Map directly preserves registration order, including numeric
scope names. Converting it to an object can reorder names and change style IDs.

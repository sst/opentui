...
`AudioStream.getStats()` returns stream state, buffered frames and duration, received bytes, decoded and played frames, underruns, and reconnect attempts. `setVolume()`, `setPan()`, and `setGroup()` remain effective after reconnecting. A naturally completed stream keeps final stats in the `ended` state until disposed.

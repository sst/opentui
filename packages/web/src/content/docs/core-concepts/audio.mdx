...
Direct reads allocate `frameCount * channels` samples, but only the first `framesRead * channels` are valid. `frameCount` cannot exceed the configured capacity. A later `startCapture()` resets the ring and counters. Successful transitions emit `captureStarted` and `captureStopped`; native failures return `false` or `null` and emit `Audio.error`. Device indices belong to the latest enumeration and can change after hot-plugging. Invalid dimensions and indices throw `TypeError` or `RangeError` before native work.

...
OpenTUI exposes a native miniaudio-backed `Audio` engine from `@lexwdex-org/core`. It can decode audio bytes or files, play sounds through named groups, choose playback devices, mix into PCM buffers, and expose recent mixed frames for visualization.

## Basic usage

```typescript
import { Audio } from "@lexwdex-org/core"

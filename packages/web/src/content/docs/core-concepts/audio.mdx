...
`playStream()` resolves after the native MP3 decoder is ready, normally while the stream is still buffering. Operational setup failures reject with `AudioStreamError`; its `context` contains the failing `action` and, when available, `status`, `errorCode`, or `attempt`. Input validation can reject with `TypeError` or `RangeError`, and cancellation uses `AbortError`. After setup, retryable failures emit `reconnecting` with an `AudioStreamError`; terminal failures emit `error` and the context separately on the returned `AudioStream`.
...
URL reconnect is opt-in. It retries network failures, interrupted response bodies, HTTP 408/425/429, and HTTP 5xx responses. Reconnects restart the decoder in the same native stream session, preserving controls, counters, the voice slot, and any decoded PCM still available after an interruption. `retryOnEnd` restarts the same session after a clean response ends and its buffered PCM drains. Initial retries happen while `playStream()` is pending and do not emit `reconnecting` because the stream has not been returned yet. `maxAttempts` limits consecutive attempts for one outage and resets after a replacement decoder becomes ready; omitting it means unlimited retries. An `AbortSignal` or `dispose()` stops them.

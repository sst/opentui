...
URL reconnect is opt-in. It retries network failures, interrupted response bodies, HTTP 408/425/429, and HTTP 5xx responses. `retryOnEnd` reconnects after a clean response ends and its buffered PCM drains. When reconnect is enabled, omitted `maxAttempts` means unlimited retries; an `AbortSignal` or `dispose()` stops them.

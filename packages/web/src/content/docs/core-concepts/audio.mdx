...
Streaming currently supports only `format: "mp3"`; that format is passed through to the native decoder and exposed as `AudioStream.format`. The option defaults to `"mp3"`, and unsupported formats reject before opening a source. Static `loadSound()` also supports WAV and FLAC. Choose the entry point by source ownership:
...
The default `contentTypePolicy: "validate"` accepts `audio/mpeg`, `audio/mp3`, `application/mp3`, `application/octet-stream`, or a missing content type. Set it to `"ignore"` to let the decoder validate mislabeled responses, or pass a callback receiving `{ format, contentType, status, url }` and return a boolean. Every response, including reconnect replacements, is evaluated before its bytes reach the decoder. `request` accepts `RequestInit` except `body` and `signal`; use the stream-level `signal` option for cancellation.
...
| ------------------- | ------- | --------------------------------------------------------- |
| `format`            | `"mp3"` | Encoded stream format; currently only MP3 is supported    |

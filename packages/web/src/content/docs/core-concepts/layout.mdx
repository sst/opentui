...
OpenTUI uses the Yoga layout engine, providing CSS Flexbox-like capabilities for responsive terminal layouts. This allows you to create complex, dynamic interfaces that adapt to terminal size changes.
...
```typescript
import { GroupRenderable, BoxRenderable, createCliRenderer } from "@opentui/core"
...
Elements are positioned relative to their parent:
...
```typescript
import { Box, Group, Text } from "@opentui/core"

...
The same layout properties apply when composing renderables manually:
...
leftPanel.add(new TextRenderable(renderer, { content: "Left Panel" }))
...
rightPanel.add(new TextRenderable(renderer, { content: "Right Panel" }))

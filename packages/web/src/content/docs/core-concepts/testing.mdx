...
`@lexwdex-org/core/testing` provides a renderer that does not write to the real terminal by default. It constructs `CliRenderer` directly with `screenMode: "main-screen"`, `consoleMode: "disabled"`, `externalOutputMode: "passthrough"`, and native memory output.

```typescript
import { createTestRenderer } from "@lexwdex-org/core/testing"
import { Text } from "@lexwdex-org/core"

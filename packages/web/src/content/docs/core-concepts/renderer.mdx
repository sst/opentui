...
| `openConsoleOnError`  | `boolean`        | `true`      | Auto-open console when errors occur          |
...
renderer.setCursorStyle("bar", true) // Blinking bar
...
Add custom input handlers that run before the default handling:
...
Use `prependInputHandler()` to add a handler at the start of the chain.

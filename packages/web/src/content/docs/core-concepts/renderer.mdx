...
const box = new BoxRenderable(renderer, { width: 40, height: 10, borderStyle: "rounded" })
box.add(new TextRenderable(renderer, { content: "Hello, OpenTUI!" }))
...
```typescript
const renderer = await createCliRenderer()
renderer.root.add(new TextRenderable(renderer, { content: "Static content" })) // Triggers render
```

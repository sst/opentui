...
| `remote`              | `boolean`                      | feed custom: `true`; otherwise auto | Override remote-terminal detection and env forwarding                                   |
...
Initial size comes from `stdout.columns` / `stdout.rows`, then `width` / `height`, then `80x24`. Feed-backed custom output defaults `remote` to `true`; process stdout and `bufferedOutput: "memory"` preserve native SSH/mosh auto-detection. Set `remote: false` only when a custom stream behaves like the local terminal and should receive default terminal env forwarding.

Remote custom streams do not forward local terminal variables by default. To apply image overrides explicitly, set `forwardEnvKeys: ["OPENTUI_GRAPHICS", "OPENTUI_IMAGE_PROTOCOL"]`.
...
Scrollback snapshots use the same image protocol resolution as live `ImageRenderable` instances. Kitty uses conventional placement, Sixel uses the renderable's detected pixel geometry, and blocks use Unicode quadrant cells. Native scrollback placement starts once the footer is pinned and the image rectangle is addressable; unsupported snapshot layouts use the normal block fallback.
...
`commitRows` throws if you call it before `render()`, or if renderer width, terminal width or height, pixel resolution, or `widthMethod` changed since the last `render()`. Re-render before committing fresh rows in either case.

For an `ImageRenderable`, await `image.loadPromise`, then call `surface.render()` and `surface.commitRows(...)`. `surface.settle()` waits for syntax highlighting, not image loading. The queued snapshot retains the image until native output publication, so the surface can be destroyed immediately after the commit.

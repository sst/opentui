...
`commitRows` throws if you call it before `render()`, or if the renderer's dimensions, pixel resolution, or `widthMethod` changed since the last `render()`. Re-render before committing fresh rows in either case.

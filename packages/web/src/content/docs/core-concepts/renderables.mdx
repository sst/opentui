...
Renderables are the building blocks of your UI - hierarchical objects that can be positioned, styled, and nested within each other. Each Renderable represents a visual element and uses the Yoga layout engine for flexible positioning and sizing.

## Creating Renderables

Renderables are created by instantiating a class with a render context (the renderer) and options:
...
- `onMouseMove`, `onMouseDrag`, `onMouseDragEnd`
...
When `visible` is `false`, the renderable is removed from Yoga layout calculation (equivalent to CSS `display: none`).
...
For animations, enable continuous rendering on a renderable:

```typescript
// Enable live mode
renderable.live = true

// In onUpdate, perform animation
renderable.onUpdate = (deltaTime) => {
  // Update animation state
  renderable.translateX += 1
}
...
Enable offscreen rendering for complex content:
...
complex.frameBuffer.fillRect(0, 0, 10, 5, RGBA.fromHex("#FF0000"))
```

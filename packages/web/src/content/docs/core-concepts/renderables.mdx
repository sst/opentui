...
```typescript
import { createCliRenderer, TextRenderable, BoxRenderable } from "@lexwdex-org/core"
...
QR code support is available from the separate `@lexwdex-org/qrcode` package.
...
```typescript
import { RenderableEvents } from "@lexwdex-org/core"

...
Most component properties accept both `RGBA` objects and color strings for convenience:
...
The `parseColor()` utility function converts various color formats to RGBA:
...
OpenTUI supports transparent cells and alpha blending for layered effects:
...
Combine colors with text attributes for styled output:

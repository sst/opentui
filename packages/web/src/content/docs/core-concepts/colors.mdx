...
## Update retained colors

Built-in renderables copy colors when you assign them. Color getters return detached values.
Changes to an input `RGBA`, a getter result, or its `buffer` do not update the retained scene.
This remains true even when another property causes a redraw.

Assign the property again to publish a new value:

```typescript
const accent = RGBA.fromHex("#22c55e")
box.backgroundColor = accent
accent.r = 1
box.backgroundColor = accent
```

The second assignment takes a new snapshot, even though it receives the same object.
The same rule applies to retained text styles, selections, placeholders, and gutter colors.
For immediate drawing, you can still reuse and mutate an `RGBA` between calls.
...
Cursor color controls use the RGB snapshot. Passing default foreground intent as a cursor color does not request
a terminal cursor-color reset.

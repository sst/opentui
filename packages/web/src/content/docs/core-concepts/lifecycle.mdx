...
`renderer.destroy()` is synchronous and idempotent. It marks the renderer as destroyed and stops input and scheduling.
Output flushing, terminal restoration, and native reclamation can finish asynchronously.
Before you reuse streams or deliberately exit after graceful shutdown, await `closed`:
...
An active frame or buffer lease can delay reclamation. A failed output endpoint rejects `closed`. Destruction alone
does not prove that restoration bytes reached the terminal.
...
- Starts ordered split-footer flushing and terminal restoration, and restores the configured `stdout.write` method.
- Emits the renderer `destroy` event.
- Releases the renderable tree, console overlay, native resources, and stream ownership as cleanup completes.
- Calls `onDestroy` after host cleanup. Use `closed` to wait for output and native teardown. OpenTUI logs callback errors.
...
During actual process exit, native rendering attempts synchronous restoration on real `process.stdout` before
termination-capable cleanup callbacks. It cannot replay an uncompleted write or finish an arbitrary asynchronous
transport after the event loop stops. This last-chance path does not replace awaiting `closed` during normal shutdown.

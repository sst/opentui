...
Every renderable's own `render()`, `renderSelf()`, `renderBefore`, and `renderAfter` drawing must stay inside its computed
layout rectangle. The renderer uses that rectangle to retain and redraw damaged rows; painting outside it can leave stale
cells. Children may draw outside a parent's rectangle when its overflow is visible. Direct frame callbacks and
postprocessors are separate drawing layers and are not subject to the renderable rectangle contract. Raw buffer-array
writes are not checked and must follow the same bounds when performed by a renderable.

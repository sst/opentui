...
OpenTUI's renderer examples use [Bun](https://bun.sh). You can import portable entry points such as `@lexwdex-org/keymap` and
`@lexwdex-org/core` in Node.js without installing Bun or enabling FFI. These imports do not create a native renderer. To
create a native renderer in Node.js, you need Node.js 26.3.0 with experimental FFI enabled.

```bash
mkdir my-tui && cd my-tui
bun init -y
bun add @lexwdex-org/core
```

## Runtime support

Importing packages does not require native FFI. For example, Node.js can load
`await import("@lexwdex-org/keymap")` and `await import("@lexwdex-org/core")` without `--experimental-ffi`.
...
```typescript
import { createCliRenderer, Text } from "@lexwdex-org/core"
...
```typescript
import { createCliRenderer, Box, Text } from "@lexwdex-org/core"
...
The OpenTUI monorepo includes `@lexwdex-org/react` and `@lexwdex-org/solid`. Other language and framework bindings live in

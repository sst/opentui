...
| `stopPropagation()`       | Stop later listeners from seeing the event              |
...
Both built-in host packages re-export the shared `@opentui/keymap` API and add the same three layers of helpers:
...
- The bare helper creates a `new Keymap(host)` with only the host installed.
...
It does not install leader tokens, ex commands, disambiguation addons, warning analyzers or textarea helpers.

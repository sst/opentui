...
- `@lexwdex-org/keymap/opentui` for terminal apps built on `CliRenderer` and `Renderable`
- `@lexwdex-org/keymap/html` for browser UIs rooted in an `HTMLElement`
...
Both built-in host packages export adapter-specific helpers. Import the shared `Keymap`, stringifiers and core types from `@lexwdex-org/keymap`.

| Package                   | Host adapter                        | Bare keymap helper              | Default helper                         |
| ------------------------- | ----------------------------------- | ------------------------------- | -------------------------------------- |
| `@lexwdex-org/keymap/opentui` | `createOpenTuiKeymapHost(renderer)` | `createOpenTuiKeymap(renderer)` | `createDefaultOpenTuiKeymap(renderer)` |
| `@lexwdex-org/keymap/html`    | `createHtmlKeymapHost(root)`        | `createHtmlKeymap(root)`        | `createDefaultHtmlKeymap(root)`        |
...
`@lexwdex-org/keymap/opentui` is the host package for terminal apps built on `@lexwdex-org/core`.
...
It does not install leader tokens, ex commands, disambiguation addons, warning analyzers, base-layout fallback or textarea helpers. Call `registerBaseLayoutFallback()` from `@lexwdex-org/keymap/addons/opentui` if you want bindings to ignore active keyboard-layout changes.
...
`@lexwdex-org/keymap/addons/opentui` adds helpers on top of the shared addon set:
...
`@lexwdex-org/keymap/html` is the host package for browser UIs rooted in an `HTMLElement` subtree.
...
```ts
import { createDefaultHtmlKeymap } from "@lexwdex-org/keymap/html"

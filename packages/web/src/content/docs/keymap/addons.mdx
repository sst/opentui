...
`@lexwdex-org/keymap` keeps the engine bare. The built-in addon packages install parser stages, field compilers, diagnostics, disambiguation and host-specific helpers on top of the public registration APIs.
...
import * as opentuiAddons from "@lexwdex-org/keymap/addons/opentui"
```
...
`@lexwdex-org/keymap/addons/opentui` re-exports every universal addon and adds the OpenTUI-specific helpers below.
...
The generated textarea bindings come from the shared `defaultTextareaKeyBindings` in `@lexwdex-org/core`. `createTextareaBindings()` prepends overrides before those defaults so custom bindings win by order.

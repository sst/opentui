...
| Export              | Description                                                                |
| ------------------- | -------------------------------------------------------------------------- |
| `commandBindings()` | Converts a command-to-key object map into the `BindingInput[]` core format |

`commandBindings()` is a pure helper. It does not register anything on the keymap.

```ts
import { commandBindings } from "@opentui/keymap/addons"
...
It trims string commands the same way `cmd` strings are normalized elsewhere, accepts string keys or keystroke objects, and lets the last normalized command win. Pass `onWarning` if you want to hear about normalized command collisions such as `" save-file "` and `"save-file"`.

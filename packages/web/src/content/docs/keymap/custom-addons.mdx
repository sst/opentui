...
Compiled metadata is exposed through projections such as `ActiveBinding.attrs`, `ActiveBinding.commandAttrs`, `ActiveKey.bindingAttrs` and `ActiveKey.commandAttrs`. `getCommands()` returns the original command objects, not a separate metadata record.
...
| `activeWhen(reactive)`  | External state can notify when it changed     | Caches `reactive.get()` until `reactive.subscribe()` fires   |
...
`require(...)` is keyed and cache-friendly. Calling `setData(name, value)` invalidates only records that depend on that data key.

Raw callback matchers re-run on every read. `ReactiveMatcher` is the cache-friendly matcher form:
...
| ------- | ---------------------------------------------------------------------------- | ----------------------------------------------- |
| Layer   | Extra layer fields feed layer compilers, parsers, expanders and transformers | No metadata output                              |
| Binding | Extra binding fields are compiler inputs only                                | `ActiveBinding.attrs`, `ActiveKey.bindingAttrs` |
| Command | Extra command properties are compiler inputs and stay on the command object   | `commandAttrs` projections                      |

Layer fields intentionally do not have `attr(...)` because there is no layer-level metadata surface on active keys, active bindings or commands.
...
| `args`                  | Current invocation args                                |
| `payload`               | Current invocation payload                             |
| `setInput(input)`       | Replace the input passed to the returned command        |
| `setArgs(args)`         | Replace the args passed to the returned command         |
| `prependArgs(args)`     | Add parsed args before existing invocation args         |
| `appendArgs(args)`      | Add parsed args after existing invocation args          |

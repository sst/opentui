...
Layer analyzers are provided by the layer-diagnostics feature pack. The default `Keymap` and built-in host helpers install it; a bare `BaseKeymap` only supports analyzers when constructed with `createLayerDiagnosticsFeature`.
...
| `getActiveKeys()` / `getGraphSnapshot()`                                   | Build hint, graph or debug UIs                                  |

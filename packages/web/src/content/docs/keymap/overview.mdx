...
All entry points use the same registration, dispatch and query model. They only differ in how they provide targets, focus and input events.
...
That field does two things: it gates the binding on `keymap.getData("vim.mode")`, and it exposes `{ mode: value }` to active-key metadata when requested.

...
experimental FFI. For details, see the [Node.js quickstart notes](/docs/getting-started/quickstart#use-nodejs).
...
```
┌─────────────────────────────────────────────────────────────────┐
│ HOST — OpenTUI, HTML, or a custom adapter                       │
│ supplies targets, focus state, key events and lifecycle         │
└────────────────────────────────────────────────┬────────────────┘
                                                 │ key input
┌────────────────────────────┐   ┌───────────────┴────────────────┐
│ REGISTER                   │   │ DISPATCH                       │
│ layers, tokens and addons  │   │ 1 run intercepts (key / raw)   │
│ compile bindings such as   │   │ 2 resolve event match keys     │
│ "ctrl+x" "dd" "<leader>s"  │   │ 3 walk layers + sequence tree  │
│ into sequence trees        │   │ 4 disambiguate exact / prefix  │
│ registerLayer()            │   │ 5 resolve command chain        │
│ registerToken()            │   │ 6 run command in app space     │
└─────────────┬──────────────┘   └───────────────┬────────────────┘
┌─────────────┴──────────────┐                   │
│ ADDONS                     │                   │
│ parsers, field compilers,  │                   │
│ command resolvers, tokens, │                   │
│ intercepts; each returns   │                   │
│ a disposer                 │                   │
└─────────────┬──────────────┘                   │
              ▼                                  ▼
┌─────────────────────────────────────────────────────────────────┐
│ LAYERS                                                          │
│ bindings: key + cmd + fields · commands: name + handler         │
│ sorted by priority; the active set follows focus, conditions,   │
│ matchers, runtime data and pending-sequence state               │
└─────────────┬──────────────────────────────────┬────────────────┘
              ▼                                  ▼
┌────────────────────────────┐   ┌────────────────────────────────┐
│ QUERY                      │   │ EVENTS                         │
│ read the current state     │   │ batched derived-state signals  │
│ getActiveKeys()            │   │ and diagnostics                │
│ getCommands()              │   │ on("state")                    │
│ getPendingSequence()       │   │ on("pendingSequence")          │
│ runCommand()               │   │ on("warning") / on("error")    │
└─────────────┬──────────────┘   └───────────────┬────────────────┘
              ▼                                  ▼
┌─────────────────────────────────────────────────────────────────┐
│ APP / UI                                                        │

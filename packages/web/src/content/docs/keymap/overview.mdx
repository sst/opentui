...
```ts
import type { Command, KeymapEvent } from "@lexwdex-org/keymap"
...
| Package                          | Description                                                                |
| -------------------------------- | -------------------------------------------------------------------------- |
| `@lexwdex-org/keymap`                | Main engine entry: `Keymap`, key stringifiers and shared types             |
| `@lexwdex-org/keymap/addons`         | Universal addons for parser stages, metadata, diagnostics and sequences    |
| `@lexwdex-org/keymap/addons/opentui` | Universal addons plus OpenTUI-specific base-layout and edit-buffer helpers |
| `@lexwdex-org/keymap/extras`         | Pure config and formatting helpers                                         |
| `@lexwdex-org/keymap/extras/graph`   | Graph snapshot helpers for debug and graph UIs                             |
| `@lexwdex-org/keymap/testing`        | Host-agnostic fake host and diagnostics for addon tests                    |
| `@lexwdex-org/keymap/opentui`        | OpenTUI host adapter for terminal apps built on `@lexwdex-org/core`            |
| `@lexwdex-org/keymap/html`           | DOM host adapter for browser UIs rooted in an `HTMLElement`                |
| `@lexwdex-org/keymap/react`          | React provider and hooks for an OpenTUI keymap                             |
| `@lexwdex-org/keymap/solid`          | Solid provider and hooks for an OpenTUI keymap                             |

Adapter entry points intentionally export adapter-specific helpers. Import the shared `Keymap`, stringifiers and core types from `@lexwdex-org/keymap`.

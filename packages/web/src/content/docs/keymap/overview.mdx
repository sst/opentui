...
[
  ...
  { key: "?", cmd: "toggle-help" },
  { key: "ctrl+k", cmd({ keymap }) { keymap.setData("palette.open", true) } },
...
- `preventDefault` controls whether the host event escapes after a match.
- `fallthrough` controls whether lower-precedence keymap candidates can continue after this binding runs.

Release bindings are supported, but only for a single stroke. Multi-stroke release bindings such as `"dd"` with `event: "release"` are rejected.

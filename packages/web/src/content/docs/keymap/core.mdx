...
The core engine only accepts full `BindingInput[]` arrays. If you want command-to-key config sugar, use `commandBindings(map)` from `@opentui/keymap` to build the array before calling `registerLayer()`.
...
Pure helper exported from `@opentui/keymap`:

...
Binding custom fields are compile inputs. They are not exposed as a public `.fields` object. If a binding-field compiler calls `attr(...)`, the compiled metadata appears as `ActiveBinding.attrs` and, when selected, `ActiveKey.bindingAttrs`.
...
| ------- | ---------------------------------------------------------------------- | -------------------------------------------------- |
| Layer   | Extra layer properties feed layer-field compilers and binding pipeline | None. Layers intentionally have no metadata output |
...
In that example, `enabled` controls availability but exposes no metadata. `title` exposes normalized metadata for command palettes and help UIs.

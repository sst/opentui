...
This page covers `@lexwdex-org/keymap` without any host-specific adapter.
...
```ts
import { Keymap, type KeymapHost } from "@lexwdex-org/keymap"
...
The core engine only accepts full `Binding[]` arrays. If you want command-to-key config sugar, use `commandBindings(map)` from `@lexwdex-org/keymap/extras` to build the array before calling `registerLayer()`.
...
Use `stringifyKeyStroke()` or `stringifyKeySequence()` for display. Canonical formatting renders `return` as `enter`; `preferDisplay: true` keeps preserved token displays such as `<leader>`. Both are exported from `@lexwdex-org/keymap`:

```ts
import { stringifyKeyStroke, stringifyKeySequence } from "@lexwdex-org/keymap"
```
...
Pure extra exported from `@lexwdex-org/keymap/extras`:
...
```ts
import { formatCommandBindings, formatKeySequence } from "@lexwdex-org/keymap/extras"
...
`getGraphSnapshot(keymap, options?)` from `@lexwdex-org/keymap/extras/graph` returns a diagnostic projection for graph visualizers and debug UIs. It includes layers, commands, bindings, sequence nodes, active keys and the current pending sequence.
...
```ts
import { getGraphSnapshot } from "@lexwdex-org/keymap/extras/graph"

...
`Keymap` is the core runtime. Layer analyzers are built in. Graph snapshots are provided by an extra helper:
...
const keymap = new Keymap(host as KeymapHost<object>)
const graph = getGraphSnapshot(keymap)
```

Warning/error reporting and layer analyzer registration are part of the core runtime because command, parser, matcher and analyzer failures use them.

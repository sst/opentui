...
| `bindings`   | `BindingInput[]` or shorthand `{ [cmd]: key }`                                               |
...
| Shorthand map   | `{ "save-file": "x", "delete-line": { name: "y", ctrl: true } }` | Expanded by `normalizeBindings()` / `registerLayer()` |
| Release binding | `{ key: "a", event: "release", ...}` | Release bindings support a single stroke only         |

Shorthand bindings are command-to-key. The object key may be any string command that `cmd` would accept, including resolver strings such as `":write session.log"`. Use full `BindingInput` objects when you need inline handlers or binding-specific fields.

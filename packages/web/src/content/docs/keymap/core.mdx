...
`Keymap` installs the graph snapshot and layer-diagnostics feature packs. `BaseKeymap` from `@opentui/keymap/base` keeps the same core runtime but leaves those feature implementations out unless you pass them explicitly:

```ts
import { BaseKeymap, type KeymapHost } from "@opentui/keymap/base"
...
const keymap = new BaseKeymap(host as KeymapHost<object>, {
  graph: createGraphFeature,
  diagnostics: createLayerDiagnosticsFeature,
})
```

Without those options, `getGraphSnapshot()` and layer-analyzer registration throw feature-not-installed errors. Warning and error reporting remains part of the core runtime because command, parser and matcher failures use it.
...
| extra fields | Feed layer-field compilers and binding compilation. Compiled layer attrs surface in graph snapshots |
...
`getGraphSnapshot(options?)` returns a diagnostic projection for graph visualizers and debug UIs. It is provided by the graph feature pack, which `Keymap` installs by default. It includes layers, commands, bindings, sequence nodes, active keys and the current pending sequence.

...
The core engine only accepts full `BindingInput[]` arrays. If you want command-to-key config sugar, use `commandBindings()` from `@opentui/keymap/addons` to build the array before calling `registerLayer()`.

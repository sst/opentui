...
`resolveBindingSections()` is for app-level config split by real registration site. Values can be `false`, a key, a full binding object, or arrays of keys/binding objects. The command name comes from the record key.

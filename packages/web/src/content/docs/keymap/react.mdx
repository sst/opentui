...
This page covers `@lexwdex-org/keymap/react`.

These bindings are for OpenTUI React apps. They consume a pre-created `Keymap<Renderable, KeyEvent>` from `@lexwdex-org/keymap/opentui`; they do not wrap the DOM/HTML adapter used in browser React apps.
...
import { KeymapProvider, useBindings } from "@lexwdex-org/keymap/react"

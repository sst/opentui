...
description: Attach, drain, and close an independent native byte feed with borrowed callback memory
skill:
...
[Renderer custom output](/docs/core-concepts/renderer#custom-streams). The renderer uses checked Session output,
not this TypeScript feed wrapper.

Direct use is for a native integration that already produces bytes through a native span-feed pointer.

## Renderer output uses Sessions
...
`CliRenderer` uses `NativeSession` to copy output tickets into host-owned storage.
The driver calls the original `stdout.write()` and acknowledges each ticket after its write callback succeeds.
It also handles output pressure and terminal restoration.

The native Session uses a feed internally. That does not make its output a borrowed `NativeSpanFeed.onData()` view.
Read [How Core uses native](/docs/native/core#complete-output-and-release-resources) for the renderer's contract.
...
These independent feed defaults do not apply to checked Session output.
Sessions use bounded output storage and reserve control capacity separately.
...
For a renderer, await `renderer.closed` after `renderer.destroy()` and before releasing its transport.
Renderer shutdown follows the Session contract, not this independent feed's `close()` method.

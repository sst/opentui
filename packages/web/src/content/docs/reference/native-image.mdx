...
Native decoding limits encoded data to 64 MiB, each dimension to 16,384 pixels, total pixels to 25 million, and decoded storage to 100 MiB. PNG `iCCP` without a supported higher-priority `cICP`, plus unsupported `cICP`/`gAMA`/`cHRM` metadata, is rejected.

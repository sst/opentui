...
`NativeImagePool` copies successive pixel frames into a bounded set of fixed-size native allocations. Each successful
publication returns an immutable `NativeImage`. Use it when a producer supplies repeated frames of the same size.
...
- When every slot is pinned, both publication methods return `null`. They do not allocate pixel storage, queue, block,
  or replace any frame. Drop the incoming frame or retry after consumers release a slot. Invalid input throws instead
  of reporting busy.
- Publication is synchronous, with no acquisition token or pending operation to cancel. To cancel input, stop calling
  either publication method. `dispose()` is idempotent, rejects future publications, and releases the pool's references
  without invalidating published frames. Those frames still require disposal.
...
`bun run bench:image-import` also compares `publishPixels()` with fresh `fromPixels()` imports and JavaScript
conversion followed by `publishRgba()`. These comparisons separate storage reuse from conversion costs.
...
- `busy` for retained native pool slots; pool publication methods return `null` rather than throwing this status

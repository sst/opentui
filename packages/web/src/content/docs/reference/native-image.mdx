...
Encoded input is limited to 64 MiB. Images and operation outputs are limited to 16,384 pixels per axis, 25 million pixels, and 100 MiB decoded storage. PNG `iCCP` and unsupported `gAMA`/`cHRM` profiles are rejected; unsupported `cICP` is treated as assumed sRGB.

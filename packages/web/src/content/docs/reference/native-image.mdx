...
Run `bun run bench:image-pool` from
[packages/core](https://github.com/anomalyco/opentui/tree/main/packages/core) to compare fresh imports with pooled frames.
It measures pixel copies, alpha checks, handle publication, and disposal. It does not create a renderer or use the
terminal or GPU. These measurements do not show end-to-end display throughput.

`bun run bench:image-import` compares `publishPixels()`, fresh `fromPixels()` imports, and JavaScript conversion
followed by `publishRgba()`. This separates storage reuse from conversion costs.

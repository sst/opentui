...
- `OPENTUI_GRAPHICS` recognizes only lowercase `false`/`0` (disable graphics detection) and `true`/`1` (automatic detection); other values leave automatic behavior unchanged. `OPENTUI_IMAGE_PROTOCOL` overrides automatic selection; forcing an unsupported protocol can produce incorrect output.

...
OpenTUI reads environment variables from `process.env`. Bun loads `.env` automatically; with Node.js, use the shell or Node's configured environment-file support. Native terminal overrides are not forwarded to remote custom streams unless named in `forwardEnvKeys`.
...
- `OPENTUI_GRAPHICS` recognizes only lowercase `false`/`0` (disable graphics detection) and `true`/`1` (automatic detection); other values leave automatic behavior unchanged. `OPENTUI_IMAGE_PROTOCOL` is case-insensitive; invalid values leave `auto` unchanged. Forcing an unsupported protocol can produce incorrect output.

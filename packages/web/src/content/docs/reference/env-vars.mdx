...
OpenTUI reads environment variables from `process.env`. Bun loads `.env` automatically; with Node.js, use the shell or Node's configured environment-file support. Remote sessions do not apply local terminal overrides unless they are named in `forwardEnvKeys`.

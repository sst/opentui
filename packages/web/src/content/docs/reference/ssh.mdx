...
`@opentui/core` is a peer dependency. The SSH package declares support for Bun >= 1.3.0 and Node.js 26.4.0. Creating its native renderer under Node also follows the [OpenTUI Node runtime requirements](/docs/getting-started#installation), including experimental FFI and any required permissions.

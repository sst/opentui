...
`@opentui/three` requires Bun >=1.3.0. Use Bun >=1.4.0 on native Windows arm64. It does not support Node.js.

The package pins Three.js 0.177.0. Its manifest lists `bun-webgpu` 0.1.7, `@dimforge/rapier2d-simd-compat`, and `planck`
as optional dependencies, but the root statically imports all three through its renderer and physics exports. They must
all resolve even when an application does not use physics. A normal install includes them. An install with optional
dependencies omitted makes the root import fail.

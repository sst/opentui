...
OpenTUI converts input in batches of at most 2048 frames. A full output ring applies backpressure before the next source chunk is requested. Native code retains no JavaScript pointers.

Source EOF automatically flushes the demuxer and conversion tail, then drains queued audio. Await `stream.closed` for terminal cleanup; there is no separate write or end call. `closed` never rejects, so attach an `error` listener to observe playback failures. It resolves after cancellation and errors too.
...
PCM uses the shared [statistics](#members-state-and-statistics): `bytesReceived` counts bytes in accepted whole input frames; `framesDecoded` counts converted engine-rate output, including the conversion tail. There is no separate input-frame counter.
...
| Layer                     | Bound                                                  |
| ------------------------- | ------------------------------------------------------ |
| Native encoded input      | `256 KiB` per encoded stream                           |
| Native output PCM         | `buffer.capacityMs`, up to `268,435,452` stereo frames |
| JavaScript PCM conversion | `2048` input frames per batch plus one partial frame   |
| JavaScript source demand  | One active `read()` or iterator `next()`               |

OpenTUI finishes the current source chunk before it requests the next chunk. A full native input ring for encoded audio, or output ring for PCM, applies backpressure to that chunk.
...
`bytesReceived` excludes framing and metadata bytes. For PCM it counts only bytes in whole input frames accepted by native conversion, not a pending partial frame. Divide by `channels * 4` using the input channel count to recover accepted input frames.
...
Non-finite PCM samples and EOF with a partial input frame terminate playback with `AudioStreamError`. PCM follows the same setup rejection and asynchronous terminal-event rules as encoded streams.

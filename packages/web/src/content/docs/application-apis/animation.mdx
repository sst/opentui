...
| `createTimeline(options?, renderer?)`  | Plays unless `autoplay` is `false`          | Uses the renderer's engine, or `engine` when omitted |
| React or Solid `useTimeline(options?)` | Plays on mount unless `autoplay` is `false` | Uses the component's renderer on mount               |

`new Timeline()` stores the `autoplay` option but does not act on it. Call `play()` and register it with `getTimelineEngine(renderer)` when you use the class directly.
...
`getTimelineEngine(renderer)` returns the engine for that renderer. It also restores the engine's frame callback if application code cleared renderer callbacks. Renderer destruction unregisters its timelines and detaches the engine.

The exported `engine` remains available for manual updates or explicit single-renderer attachment. Omitting the renderer from `createTimeline()` uses this default engine; it never selects the last-created renderer. Detach before explicitly moving an engine to a different renderer. A timeline can belong to only one engine at a time.

| Method                 | Behavior                                                     |
| ---------------------- | ------------------------------------------------------------ |
| `attach(renderer)`     | Installs a frame callback; rejects a different active owner  |
...
| `clear()`              | Unregisters all timelines from this engine                   |
| `update(deltaTime)`    | Updates every registered timeline that is not synced         |

`attach()` checks already-playing timelines and updates the renderer's live state. You do not need an explicit attach call when using `getTimelineEngine(renderer)` or `createTimeline(options, renderer)`.
...
3. Call `engine.detach()` only when this owner explicitly controls the whole engine.
...
Both framework packages test independent renderer ownership and hook cleanup.
...
OpenTUI no longer installs or replaces process-global `requestAnimationFrame`, `cancelAnimationFrame`, or `window` properties. Migrate terminal applications to the renderer methods; browser animation APIs are unchanged.

The shared Session scheduler services ready work between callbacks and parks Sessions awaiting output or host replies. It cannot preempt a synchronous JavaScript callback. Timeline item counts and callback execution time still affect service delay.

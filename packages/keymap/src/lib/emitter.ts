...
export interface EmitterApi<TEvents extends Record<string, unknown>> {
...
export function createEmitter<TEvents extends Record<string, unknown>>(
  ...
  let listeners: EmitterListeners<TEvents> = Object.create(null) as EmitterListeners<TEvents>
  const off = <TName extends keyof TEvents>(name: TName, listener: EmitterListener<TEvents[TName]>): void => {
  ...
  return {
    hook(name, listener) {
      const current = listeners[name] ?? []
      listeners[name] = [...current, listener] as readonly EmitterListener<TEvents[typeof name]>[]
    ...
    off,
    clear() {
      listeners = Object.create(null) as EmitterListeners<TEvents>
    },
    emit(name, ...args) {
      const current = listeners[name] as readonly EmitterListener<TEvents[typeof name]>[] | undefined
      if (!current || current.length === 0) {
        return
      }
      ...
      for (const listener of current) {
        try {
          if (args.length === 0) {
            ...
            ;(listener as (value: TEvents[typeof name]) => void)(args[0] as TEvents[typeof name])
          }
        } catch (error) {
          onError(name, error)
        }

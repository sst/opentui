...
console.log("Building @opentui/latex library...")
...
const buildResult = await Bun.build({
  entrypoints: [
    ...
    join(rootDir, "src/react.ts"),
    join(rootDir, "src/solid.ts"),
    join(rootDir, "src/markdown.ts"),
  ],
  target: "bun",
  format: "esm",
  outdir: distDir,
...
if (existsSync(corePackageJsonPath)) {
  ...
  if (coreBuildResult.status !== 0) {
    console.error("Error: Failed to build @opentui/core declarations required by @opentui/latex")
    process.exit(1)
  }
}

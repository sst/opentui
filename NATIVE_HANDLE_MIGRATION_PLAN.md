...
### Borrowed or Temporary Pointers
...
Use a generational handle table owned by `lib.zig`.

### Handle Type

Use `u64` for all object handles.
...
FFI declarations should use integer ABI types for handles. Prefer `u64` if Bun FFI supports round-tripping the values safely in this project. If JavaScript number precision is a concern, use `u32` handles initially or use `bigint` consistently. A `u32` handle can still encode slot and generation if the table size is bounded, for example 16 bits index plus 15 bits generation plus invalid zero.
...
4. Update `packages/core/src/zig.ts` FFI declarations and `FFIRenderLib` methods to use `RendererHandle`.
5. Update TypeScript classes so `rendererPtr` becomes a handle, not a `Pointer`.
...
7. Run renderer and keymap integration tests with `OTUI_DEBUG_FFI=1` to verify there are no FFI calls using a destroyed renderer pointer.

### Phase 4: Migrate Buffer Handles

1. Migrate `OptimizedBuffer` / frame buffer exports to handles.
2. Decide whether `frame_buffer` is a distinct handle kind or an alias of `optimized_buffer`.
3. Update all buffer draw/copy/read APIs to resolve source and target handles.
4. Invalid source buffer should make draw APIs no-op; invalid target buffer should no-op or return an error depending on current API shape.
5. Add tests for stale source and stale target handles.
6. Benchmark frame buffer draw and resolved-char write hot paths.
...
6. Re-run the original cursor use-after-destroy scenario. It must be impossible for the stale renderer handle or stale editor resources to crash.
...
3. Native callbacks should pass the stream handle, not a raw stream pointer, if JavaScript needs to dispatch by stream identity.
4. If native callbacks currently only know the stream pointer, store the handle in the stream object or in callback context during creation.
5. Add tests:
...
6. Benchmark stream write/commit/drain paths.
...
Confirm debug logs do not contain object method calls after the corresponding destroy call for the same underlying native object. During migration this may require adding debug output for handle invalidation rather than raw pointer calls.
